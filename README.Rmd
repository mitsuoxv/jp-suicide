...
```{r unemployment_chart, echo=FALSE, message=FALSE}
unemployment_rate <- read_csv("data/unemployment_rate.csv")

names(unemployment_rate) <- c("time", "area_code", "area", "percent", "note")

unemployment_rate %>% 
  select(c(time, percent)) %>% 
  mutate(
    time = str_replace(time, "年", "-"),
    time = str_replace(time, "月", "-01"),
    time = as_date(time)
    ) %>% 
